...
//when (k<FK) sartu[s]
	public synchronized int sartu(int id, String spaceP) throws InterruptedException {
		...
		//Ez da notify behar return-a rekin notify() egingo balitz bezala da.
		return kont-1;
	}
...
//when(x==a) atera[a]
	public synchronized void irten(int id, String spaceP, int prz) throws InterruptedException {
		...
		p.idatzi(this.toString(id));
		notifyAll();//Itxaroten dauden hari guztiei abisatu, atera edo sar daitezen.
		//notify izango balitz ateratzen zen bakoitzean beste prozesu bat sartuko litzateke, eta bizitasun propietatea
		//emango litzateke
	}
