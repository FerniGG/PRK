...
public class BasatiakApp {
	...
	public final static int zenbatBotaMax = 3;
	...
	public static void main(String args[]) {
		...
		Sukaldaria s = new Sukaldaria(zenbatBotaMax, k, pant);
		s.start();
...
class Sukaldaria extends Thread {
	...
	Sukaldaria(int max, Kontrolatzailea k, Pantaila pp) {
		zenbatbotamax = max;
		kontrol = k;
		p = pp;
	}
	...
	public void run() {
		...
		try {
			while (true) {
				unekozenbat= getRandomNumberInRange(1,this.zenbatbotamax);
				kontrol.LapikoaBete(unekozenbat);
...
class Kontrolatzailea {
	...
	synchronized void LapikoaBete(int zenbat) throws InterruptedException {
		...
		pantaila.bete(this.unekop,zenbat);
		notify();
	}
...
class Pantaila {
	...
	synchronized public void bete(int unekoak,int zenbat) {
		System.out.print("bota "+zenbat);
		margotuLapikoa(unekoak, -1);
	}
